...
from tools.skeleton.converter import ntu_to_coco
...
class TransformConvention:
    def __init__(self, src_convention='nturgb+d', tgt_convention='coco'):
        self.src_convention = src_convention
        self.tgt_convention = tgt_convention

    def __call__(self, results):
        keypoints = results['keypoint']

        if self.src_convention == 'nturgb+d' and self.tgt_convention == 'coco':
            keypoints = ntu_to_coco(keypoints)
        else:
