...
class RandomRot:
    ...
    def __init__(self, theta=0.3, num_duplicates=1):
        self.theta = theta
    ...
    def __call__(self, results):
        ...
        for _ in range(self.num_duplicates):
            ...
            skeleton_rot = np.einsum('ab,mtvb->mtva', rot_mat, skeleton)
            joints.append(skeleton_rot)

        results['joints'] = np.concatenate(joints)
        return results

class RandomRotOneAxis:

    def __init__(self, theta=0.3, axis=1, append_index=False, num_duplicates=1):
        self.theta = theta
        self.axis = axis
    ...
    def __call__(self, results):
        ...
        for _ in range(self.num_duplicates):
            ...
            if self.append_index:
                ...
                indexes.append(np.array([angles_list.index(closest_angle)], dtype=np.int64))

        results['joints'] = np.concatenate(joints)
        results['indexes'] = np.concatenate(indexes)
