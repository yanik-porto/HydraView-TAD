...
class DatasetMV(Dataset):
    def __init__(self, data_path, dataset_name, split='xsub_train', n_views=2, num_classes=120, classes_map=None, preprocessing=False,
                 ...
                 text_embeds_suffix='_clip.npy', is_dynamic_loading=False, do_save_indexes=False):
        ...
        self.do_save_indexes = do_save_indexes
    ...
    def load_data(self, data_path):
        ...
        if self.do_save_indexes:
            self.save_indexes(data)
    ...
    def save_indexes(self, data):
        for idx in range(len(data)):
            _, camIdx = self.names_splitter.get_motionid_from_name(data[idx]["frame_dir"])
            data[idx]['indexes'] = np.array([camIdx], dtype=np.int64)
    ...
    def find_associated_ids(self, data):
        ...
        # sort indexes by groups
        groups = {}
        for didx, d in enumerate(data):
            group, _ = self.get_group_from_name(d["frame_dir"])
            if not group in groups:
                groups[group] = []
            groups[group].append(didx)

        for idx in range(len(data)):
            assocs = self.get_associated_ids(idx, data, groups)
    ...
    # TODO : check compatibility with derived methods and groups argument
    def get_associated_ids(self, idx, data, groups):
        assocs = []
    ...
    def get_associated_ids_old(self, idx, data):
        assocs = []
    ...
    def merge_annot_into_other(self, annot, other):
        ...
        if 'indexes_optim' in annot and 'indexes_optim' in concat:
            concat['indexes_optim'] = np.concatenate((concat['indexes_optim'], annot['indexes_optim']))
