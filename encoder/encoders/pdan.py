...
class PDAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if P != 1:
            ...
            x = pool_person(x)
            x = x.reshape(B, C, T, 1, 1)
        x = x.squeeze(3).squeeze(3)

        mask_list = torch.sum(mask, 1)
        mask_new = np.zeros((mask.size()[0], self.num_classes, mask.size()[1]))
        for i in range(mask.size()[0]):
            mask_new[i, :, :int(mask_list[i])] = np.ones((self.num_classes, int(mask_list[i])))
        mask_new = torch.from_numpy(mask_new).float()
        mask_new = Variable(mask_new.to(x.get_device()))

        out = self.stage1(x, mask_new)
        outputs = out.unsqueeze(0)
        for s in self.stages:
            out = s(out * mask_new[:, 0:1, :], mask_new)
